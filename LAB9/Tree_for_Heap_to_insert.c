...
void InputRecord(Record element[], int* count){
    if(*count < MAX_RECORD){
        ...
        if(newData.who[0] >= 'a' && newData.who[0] <= 'z')
            newData.who[0] = toupper(newData.who[0]);
...
void DisplayRecord(Record element[], int count){
    ...
    for(int i = 0; i < count; i++){
        printf("%d\n%s, %s, %s, %s\n", element[i].priority, element[i].who, element[i].event, element[i].date, element[i].location);
    }
}

void popHeap(Record element[], int *count) {
    if (*count == 0) {
        printf("No Data\n");
        return;
    }
    printf("%d\n%s, %s, %s, %s\n", element[0].priority, element[0].who, element[0].event, element[0].date, element[0].location);
    element[0] = element[*count - 1];
    (*count)--;
    heapify(element, *count, 0);
}
